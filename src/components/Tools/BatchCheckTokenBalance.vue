<template>
  <div class="batchTokenImputation batchCheckTokenBalance">
    ...
    <div class="title display-flex align-items-center justify-content-sp">
      <span class="display-flex align-items-center">
        ...
        <el-tooltip
          effect="light"
          :content="i18n.t('tool.AutofillChainID')"
    ...
    <div class="setting-box display-flex align-items-center justify-content-sp">
      <div class="setting-item display-flex flex-direction-col">
        <div class="title display-flex align-items-center justify-content-sp">
          <span>{{ i18n.t('tool.Mode') }}</span>
        </div>
        <div class="setting-mode display-flex align-items-center justify-content-sp">
          <span
            v-for="(item, index) in modeList"
            :key="index"
            :class="modeIndex == item.id ? 'active' : ''"
            >{{ item.text }}</span
          >
        </div>
      </div>
      <div class="setting-item display-flex flex-direction-col">
        <div class="title display-flex align-items-center justify-content-sp">
          <span>{{ i18n.t('tool.TokenType') }}</span>
        </div>
        <div class="setting-token display-flex align-items-center justify-content-sp">
          <span
            v-for="(item, index) in tokenTypeList"
            :key="index"
            :class="tokenTypeIndex == item.id ? 'active' : ''"
            >{{ item.text }}</span
          >
        </div>
      </div>
    ...
    <div class="btn">{{ i18n.t('tool.CheckBalance') }}</div>
  </div>
</template>
...
const modeIndex = ref<number>(1)
const modeList = [
  {
    id: 1,
    text: i18n.t('tool.ExtremeSpeed')
  },
  {
    id: 2,
    text: i18n.t('tool.Normal')
  }
]

const tokenTypeIndex = ref<number>(1)
const tokenTypeList = [
  {
    id: 1,
    text: 'ERC20'
